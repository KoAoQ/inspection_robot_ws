...
namespace lslidar_driver {
        ...
        std::string lidar_model)
    {
        ...
        // M10P的第2、3字节表示当前数据包长度
        if (lidar_model == "M10P") {
            while (count < 4 && rclcpp::ok()) {
                const auto n = read(
                    &packet->data[count],
                    static_cast<size_t>(4 - count));
            ...
            packet_length =
                (static_cast<int>(packet->data[2]) << 8) |
                static_cast<int>(packet->data[3]);

            if (packet_length < 20 || packet_length > 180) {
                return 0;
            }
        }

        // 读取这一帧剩余的数据
        while (count < packet_length && rclcpp::ok()) {
            const auto n = read(
                &packet->data[count],
                static_cast<size_t>(packet_length - count));
