...
enum class BaseState : std::uint8_t {
  kDisconnected = 0,
  kWaitingForFeedback = 1,
  kReady = 2,
  kFaultLatched = 3
};
...
struct EvaluationResult {
  bool feedback_timeout_transition{false};
};
...
class BaseFaultManager {
 public:
  ...
  BaseFaultManager(std::chrono::milliseconds command_timeout,
  ...
  bool faultLatched() const {
    return state_ == BaseState::kFaultLatched;
  }
